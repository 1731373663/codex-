...
type Route = "overview" | "relay" | "relayEnvironment" | "sessions" | "context" | "enhance" | "dreamSkin" | "zedRemote" | "userScripts" | "recommendations" | "maintenance" | "about" | "settings" | "claudeProviders";
type Theme = "dark" | "light";

const routes: Array<{ id: Route; label: string; icon: LucideIcon; badge?: string }> = [
  { id: "overview", label: t("概览"), icon: LayoutDashboard },
  { id: "relay", label: t("供应商配置"), icon: KeyRound },
  { id: "claudeProviders", label: t("Claude 供应商"), icon: KeyRound },
...
export function App() {
  ...
  const [claudeSwitching, setClaudeSwitching] = useState(false);
  const [claudeStatus, setClaudeStatus] = useState<{ activeTokenPreview: string; activeBaseUrl: string; activeModelHaiku: string; activeModelSonnet: string; activeModelOpus: string; activeModel: string; settingsExists: boolean } | null>(null);
  const [claudeCcsProviders, setClaudeCcsProviders] = useState<{ dbPath: string; providers: Array<{ sourceId: string; name: string; baseUrl: string; authTokenPreview: string }> } | null>(null);
  ...
  const refreshClaudeStatus = async (silent = false) => {
    type ClaudeStatusResponse = { status: string; message: string; claudeStatus: { activeTokenPreview: string; activeBaseUrl: string; activeModelHaiku: string; activeModelSonnet: string; activeModelOpus: string; activeModel: string; settingsExists: boolean } };
    const result = await run(() => call<ClaudeStatusResponse>("load_claude_status"));
    if (result) {
      if (result.claudeStatus) setClaudeStatus(result.claudeStatus);
      if (!silent) showResultNotice(t("Claude 供应商"), result, { silentSuccess: true });
    }
  };

  const refreshClaudeCcsProviders = async (silent = false) => {
    type CcsResponse = { status: string; message: string; dbPath: string; providers: Array<{ sourceId: string; name: string; baseUrl: string; authTokenPreview: string }> };
    const result = await run(() => call<CcsResponse>("load_claude_ccs_providers"));
    if (result) {
      setClaudeCcsProviders({ dbPath: result.dbPath, providers: result.providers });
      if (!silent) showResultNotice(t("Claude cc-switch"), result, { silentSuccess: true });
    }
  };

  const importClaudeCcsProviders = async () => {
    const result = await run(() => call<SettingsResult>("import_claude_ccs_providers"));
    if (result) {
      setSettings(result);
      setSettingsForm(normalizeSettings(result.settings));
      showResultNotice(t("Claude cc-switch 导入"), result);
      await refreshClaudeCcsProviders(true);
    }
  };

  const switchClaudeProfile = async (settings: BackendSettings) => {
    setClaudeSwitching(true);
    try {
      const result = await run(() => call<SettingsResult>("switch_claude_profile", { settings }));
      if (result) {
        setSettings(result);
        setSettingsForm(normalizeSettings(result.settings));
        showResultNotice(t("Claude 供应商切换"), result);
        await refreshClaudeStatus(true);
      }
    } finally {
      setClaudeSwitching(false);
    }
  };

  const testClaudeProfile = async (profile: ClaudeProfile) => {
    type TestResponse = { status: string; message: string };
    const result = await run(() => call<TestResponse>("test_claude_profile", { profile }));
    ...
    <div className={`shell ${theme}`}>
      ...
      <main className="workspace">
        ...
        <section className="screen" key={route}>
          ...
          {route === "claudeProviders" ? (
            <ClaudeProvidersScreen
              form={settingsForm}
              onFormChange={setSettingsForm}
              claudeStatus={claudeStatus}
              claudeCcsProviders={claudeCcsProviders}
              actions={actions}
            />
          ) : null}
...
type ClaudeStatusState = {
  ...
  activeModelHaiku: string;
  activeModelSonnet: string;
  activeModelOpus: string;
  activeModel: string;
  settingsExists: boolean;
};

type ClaudeCcsProvidersState = {
  dbPath: string;
  providers: Array<{ sourceId: string; name: string; baseUrl: string; authTokenPreview: string }>;
};

function emptyClaudeProfile(): ClaudeProfile {
  return {
    id: `claude-${Date.now()}`,
    name: "",
    baseUrl: "",
...
function ClaudeProvidersScreen({
  form,
  onFormChange,
  claudeStatus,
  claudeCcsProviders,
  ...
  form: BackendSettings;
  onFormChange: (value: BackendSettings) => void;
  claudeStatus: ClaudeStatusState | null;
  claudeCcsProviders: ClaudeCcsProvidersState | null;
  actions: Actions;
}) {
  const profiles = form.claudeProfiles ?? [];
  ...
  const saveEdit = () => {
    ...
    onFormChange({
      ...form,
      claudeProfiles: profiles.map((p) => (p.id === editDraft.id ? editDraft : p)),
    });
    setEditingId(null);
    setEditDraft(null);
  };
  ...
  const addProfile = () => {
    const newProfile = emptyClaudeProfile();
    onFormChange({ ...form, claudeProfiles: [...profiles, newProfile] });
  ...
  const deleteProfile = (id: string) => {
    ...
    onFormChange({
      ...form,
      claudeProfiles: next,
      activeClaudeProfileId: form.activeClaudeProfileId === id ? (next[0]?.id ?? "") : form.activeClaudeProfileId,
    });
    if (editingId === id) cancelEdit();
  };

  const selectProfile = (id: string) => {
    onFormChange({ ...form, activeClaudeProfileId: id });
  };

  const doSwitch = async () => {
    await actions.switchClaudeProfile(form);
  };

  const doTest = async (profile: ClaudeProfile) => {
    await actions.testClaudeProfile(profile);
  };

  const currentProfile = profiles.find((p) => p.id === activeId) ?? null;
  ...
  return (
    <>
      ...
      <Panel>
        <CardHead
          title={t("Claude Code 供应商")}
          detail={
            claudeStatus?.settingsExists
              ? tf("当前：{0}  {1}", [claudeStatus.activeBaseUrl || t("官方地址"), claudeStatus.activeModel || "-"])
              : t("~/.claude/settings.json 尚未检测到")
          }
        />
        <CardContent>
          {claudeStatus ? (
            <div className="metric-list">
              <Metric label={t("Base URL")} value={claudeStatus.activeBaseUrl || t("（默认）")} />
              <Metric label={t("Token")} value={claudeStatus.activeTokenPreview || t("（未设置）")} />
              <Metric label={t("Haiku 模型")} value={claudeStatus.activeModelHaiku || "-"} />
              <Metric label={t("Sonnet 模型")} value={claudeStatus.activeModelSonnet || "-"} />
              <Metric label={t("Opus 模型")} value={claudeStatus.activeModelOpus || "-"} />
              <Metric label={t("默认模型")} value={claudeStatus.activeModel || "-"} />
            </div>
          ) : (
            <p className="field-hint">{t("点击「刷新」读取当前 ~/.claude/settings.json 状态。")}</p>
          )}
          <Toolbar>
            <Button onClick={() => void actions.refreshClaudeStatus(false)}><RefreshCw className="h-4 w-4" />{t("刷新状态")}</Button>
          </Toolbar>
        </CardContent>
      </Panel>
      ...
      {/* 供应商列表 */}
      <Panel>
        <CardHead title={t("供应商配置列表")} detail={tf("{0} 个配置，当前激活：{1}", [profiles.length, currentProfile?.name || t("（未选择）")])} />
        <CardContent>
          {profiles.length === 0 ? (
            <p className="field-hint">{t("暂无供应商配置，点击「添加」新建一个。")}</p>
          ) : (
            <div className="status-table">
              {profiles.map((profile) => (
                <div key={profile.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
                  <span style={{ flex: 1, fontWeight: profile.id === activeId ? 600 : undefined }}>
                    {profile.name || t("（未命名）")}
                    {profile.id === activeId ? <span style={{ marginLeft: 6, fontSize: "0.75em", opacity: 0.6 }}>✓ {t("激活")}</span> : null}
                  </span>
                  <span style={{ fontSize: "0.8em", opacity: 0.6, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {profile.baseUrl || t("官方地址")}
                  </span>
                  <Button variant="secondary" onClick={() => selectProfile(profile.id)}>{t("选择")}</Button>
                  <Button variant="secondary" onClick={() => startEdit(profile)}><Edit3 className="h-4 w-4" /></Button>
                  <Button variant="secondary" onClick={() => void doTest(profile)}><TestTube className="h-4 w-4" /></Button>
                  <Button variant="secondary" onClick={() => deleteProfile(profile.id)}><Trash2 className="h-4 w-4" /></Button>
                </div>
              ))}
            </div>
          )}
          <Toolbar>
            <Button onClick={() => addProfile()}><Plus className="h-4 w-4" />{t("添加供应商")}</Button>
            <Button
              onClick={() => void doSwitch()}
              disabled={!currentProfile || actions.claudeSwitching}
            >
              {actions.claudeSwitching ? <RefreshCw className="h-4 w-4" /> : <Play className="h-4 w-4" />}
              {t("切换到选中供应商")}
            </Button>
            <Button variant="secondary" onClick={() => void actions.saveSettingsValue(form)}>{t("保存到 Codex++")}</Button>
          </Toolbar>
        </CardContent>
      </Panel>
      ...
      {editDraft && editingId ? (
        <Panel>
          <CardHead title={t("编辑供应商配置")} detail={editDraft.name || t("（新配置）")} />
          <CardContent>
            <div className="form-row">
              <Field label={t("名称")}>
                <Input
                  value={editDraft.name}
                  placeholder={t("例如：模型港")}
            ...
            <div className="form-row">
              <Field label={t("Base URL")}>
                <Input
                  ...
                  placeholder="https://moxinggang.com"
                  spellCheck={false}
                  onChange={(e) => setEditDraft({ ...editDraft, baseUrl: e.currentTarget.value.trim() })}
                />
              </Field>
            </div>
            <div className="form-row">
              <Field label={t("API Token (ANTHROPIC_AUTH_TOKEN)")}>
                <Input
                  value={editDraft.authToken}
                  type="password"
                  placeholder="sk-ant-..."
                  spellCheck={false}
                  onChange={(e) => setEditDraft({ ...editDraft, authToken: e.currentTarget.value.trim() })}
                />
              </Field>
            </div>
            <div className="form-row">
              <Field label={t("Haiku 模型别名")}>
                <Input
                  value={editDraft.modelHaiku}
                  placeholder="claude-haiku-4-5-20251001"
                  spellCheck={false}
                  onChange={(e) => setEditDraft({ ...editDraft, modelHaiku: e.currentTarget.value.trim() })}
                />
              </Field>
              <Field label={t("Sonnet 模型别名")}>
                <Input
                  value={editDraft.modelSonnet}
                  placeholder="claude-sonnet-5"
                  spellCheck={false}
                  onChange={(e) => setEditDraft({ ...editDraft, modelSonnet: e.currentTarget.value.trim() })}
                />
              </Field>
              <Field label={t("Opus 模型别名")}>
                <Input
                  value={editDraft.modelOpus}
                  placeholder="claude-opus-5"
                  spellCheck={false}
                  onChange={(e) => setEditDraft({ ...editDraft, modelOpus: e.currentTarget.value.trim() })}
                />
              </Field>
            </div>
            <div className="form-row">
              <Field label={t("默认模型")}>
                <AppSelect
                  ...
                  onChange={(v) => setEditDraft({ ...editDraft, defaultModel: v })}
                  options={[
            ...
            <Toolbar>
              <Button onClick={saveEdit}><Save className="h-4 w-4" />{t("保存")}</Button>
              <Button variant="secondary" onClick={cancelEdit}>{t("取消")}</Button>
            </Toolbar>
          </CardContent>
        </Panel>
      ) : null}
      ...
      <Panel>
        <CardHead
          title={t("从 cc-switch 导入")}
          detail={
            claudeCcsProviders
              ? tf("检测到 {0} 个 Claude 类供应商", [claudeCcsProviders.providers.length])
              : t("点击「扫描」读取 cc-switch 数据库")
          }
        />
        <CardContent>
          {claudeCcsProviders && claudeCcsProviders.providers.length > 0 ? (
            <div className="status-table">
              {claudeCcsProviders.providers.map((p) => (
                <div key={p.sourceId} style={{ display: "flex", gap: 8, padding: "3px 0" }}>
                  <span style={{ flex: 1 }}>{p.name}</span>
                  <span style={{ flex: 2, fontSize: "0.8em", opacity: 0.7 }}>{p.baseUrl}</span>
                  <span style={{ fontSize: "0.8em", opacity: 0.5 }}>{p.authTokenPreview || t("无 token")}</span>
                </div>
              ))}
            </div>
          ) : claudeCcsProviders ? (
            <p className="field-hint">{t("cc-switch 数据库中没有找到 Claude 类供应商（app_type = 'claude'）。")}</p>
          ) : null}
          <Toolbar>
            <Button variant="secondary" onClick={() => void actions.refreshClaudeCcsProviders(false)}>
              <RefreshCw className="h-4 w-4" />{t("扫描 cc-switch")}
            </Button>
            <Button
              onClick={() => void actions.importClaudeCcsProviders()}
              disabled={!claudeCcsProviders || claudeCcsProviders.providers.length === 0}
            >
              <Download className="h-4 w-4" />{t("导入全部（去重）")}
            </Button>
          </Toolbar>
          <p className="field-hint">
            {t("cc-switch 数据库路径：")}{claudeCcsProviders?.dbPath ?? t("默认路径")}
          </p>
        </CardContent>
      </Panel>
    </>
