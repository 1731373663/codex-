...
/// Payload returned by load_claude_status
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ClaudeStatusPayload {
    /// Renamed to avoid conflict with CommandResult's own `status` field
    pub claude_status: codex_plus_core::claude_config::ClaudeStatus,
}

/// Payload returned by load/import claude ccs providers
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ClaudeCcsProvidersPayload {
    pub db_path: String,
    pub providers: Vec<ClaudeImportItem>,
}

/// One importable provider row shown in the UI
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ClaudeImportItem {
    pub source_id: String,
    pub name: String,
    pub base_url: String,
    /// First 8 chars of auth_token + "…", empty when absent
...
/// 读取 ~/.claude/settings.json 的当前状态（不含原始 token）
#[tauri::command]
pub fn load_claude_status() -> CommandResult<ClaudeStatusPayload> {
    let claude_status = codex_plus_core::claude_config::read_claude_status();
    ok("Claude 供应商状态已读取。", ClaudeStatusPayload { claude_status })
}

/// 将指定 ClaudeProfile 写入 ~/.claude/settings.json 并更新 activeClaudeProfileId
#[tauri::command]
pub fn switch_claude_profile(settings: BackendSettings) -> CommandResult<SettingsPayload> {
    let store = SettingsStore::default();
    let settings = normalize_settings_before_save(settings);
    ...
    let Some(profile) = settings
        .claude_profiles
        .iter()
        .find(|p| p.id == settings.active_claude_profile_id)
        .cloned()
    else {
        return failed(
            "找不到指定的 Claude 供应商配置，请检查 ID 是否正确。",
            settings_payload_value().unwrap_or_else(|(_, p)| p),
        );
    };

    if let Err(error) = codex_plus_core::claude_config::apply_claude_profile(&profile) {
        return failed(
            &format!("写入 ~/.claude/settings.json 失败：{error}"),
            settings_payload_value().unwrap_or_else(|(_, p)| p),
        );
    }
    ...
    match store.save(&settings) {
        Ok(()) => settings_payload(
            &format!("已切换到 Claude 供应商：{}。", profile.name),
            "切换后重新读取设置失败",
        ),
        Err(error) => failed(
            &format!("保存 Codex++ 设置失败：{error}"),
            settings_payload_value().unwrap_or_else(|(_, p)| p),
        ),
    }
}

/// 测试指定 ClaudeProfile 是否能正常连接（向 /v1/models 发一个 GET 请求）
#[tauri::command]
pub async fn test_claude_profile(
    profile: codex_plus_core::claude_config::ClaudeProfile,
) -> CommandResult<Value> {
    match codex_plus_core::claude_config::test_claude_provider_connection(
        &profile.base_url,
        &profile.auth_token,
    )
    .await
    {
        Ok(code) => {
            if code < 500 {
                ok(&format!("连接成功（HTTP {code}）。"), json!({ "httpStatus": code }))
            } else {
                failed(&format!("服务器错误（HTTP {code}）。"), json!({ "httpStatus": code }))
            }
        }
...
/// 从 cc-switch 数据库读取 Claude 类型供应商（app_type = 'claude'）
#[tauri::command]
pub fn load_claude_ccs_providers() -> CommandResult<ClaudeCcsProvidersPayload> {
    let db_path = codex_plus_core::ccs_import::default_ccs_db_path();
    let rows = match list_claude_providers_from_db(&db_path) {
        Ok(r) => r,
        Err(e) => {
            return failed(
                &format!("读取 cc-switch Claude 供应商配置失败：{e}"),
                ClaudeCcsProvidersPayload {
                    db_path: db_path.to_string_lossy().to_string(),
                    providers: Vec::new(),
                },
            )
    ...
    let providers = rows
        .iter()
        .map(|(sid, name, config)| {
            let env = config.get("env").and_then(|v| v.as_object()).cloned().unwrap_or_default();
            let token = env.get("ANTHROPIC_AUTH_TOKEN")
                .and_then(|v| v.as_str())
                .or_else(|| config.get("apiKey").and_then(|v| v.as_str()))
                .unwrap_or("");
            let preview = if token.is_empty() {
                String::new()
            } else if token.len() <= 8 {
                format!("{token}…")
            } else {
            ...
            let base_url = env.get("ANTHROPIC_BASE_URL")
                ...
                .or_else(|| config.get("baseURL").and_then(|v| v.as_str()))
                .unwrap_or("")
                .trim_end_matches('/')
                .to_string();
            ClaudeImportItem {
        ...
        .collect();
    ok(
        &format!("已读取 cc-switch Claude 供应商配置：{} 个。", rows.len()),
        ClaudeCcsProvidersPayload {
            db_path: db_path.to_string_lossy().to_string(),
            providers,
        },
...
/// 将 cc-switch 中的 Claude 供应商批量导入 Codex++ 设置（去重）
#[tauri::command]
pub fn import_claude_ccs_providers() -> CommandResult<SettingsPayload> {
    let db_path = codex_plus_core::ccs_import::default_ccs_db_path();
    let rows = match list_claude_providers_from_db(&db_path) {
        Ok(r) => r,
        Err(e) => {
            let payload = settings_payload_value().unwrap_or_else(|(_, p)| p);
            return failed(&format!("读取 cc-switch Claude 供应商配置失败：{e}"), payload);
        }
    };
    ...
    let mut existing_keys: Vec<String> = settings
        .claude_profiles
        .iter()
        .map(codex_plus_core::claude_config::claude_profile_identity)
        .collect();
    let mut existing_ids: Vec<String> = settings
        .claude_profiles
        .iter()
    ...
    for (source_id, name, config) in &rows {
        let base_url = config
            .get("env").and_then(|e| e.get("ANTHROPIC_BASE_URL")).and_then(|v| v.as_str())
            .or_else(|| config.get("base_url").and_then(|v| v.as_str()))
            .or_else(|| config.get("baseURL").and_then(|v| v.as_str()))
            .unwrap_or("")
            .trim_end_matches('/')
            .to_string();
        let key = codex_plus_core::claude_config::claude_import_key_from_raw(name, &base_url);
        if existing_keys.iter().any(|k| k == &key) {
            continue;
        }
        if let Some(profile) = codex_plus_core::claude_config::claude_profile_from_ccs_value(
            source_id,
            name,
            config,
            &existing_ids,
            ...
            existing_ids.push(profile.id.clone());
            existing_keys.push(key);
            settings.claude_profiles.push(profile);
            imported += 1;
        }
    }

    if imported == 0 {
        return settings_payload("没有新的 cc-switch Claude 供应商配置需要导入。", "设置读取失败");
    }

    settings = normalize_settings_before_save(settings);
    match store.save(&settings) {
        Ok(()) => settings_payload(
            &format!("已从 cc-switch 导入 Claude 供应商配置：{imported} 个。"),
            "导入后重新读取设置失败",
        ),
        Err(e) => failed(
            &format!("保存设置失败：{e}"),
...
/// 内部：委托给 codex_plus_core::claude_config 读取 cc-switch Claude 供应商
fn list_claude_providers_from_db(
    path: &std::path::Path,
) -> anyhow::Result<Vec<(String, String, serde_json::Value)>> {
    codex_plus_core::claude_config::list_claude_providers_from_ccs_db(path)
}
